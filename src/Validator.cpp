...
// Items sitting flush against each other are fine, so the comparison is strict on every
// axis. Only a genuine shared volume counts as an overlap.
...
ValidationResult Validator::validate(
    ...
    // Walk every placement once, checking it on its own, and gather the placements into the
    // box instance they landed in so the box wide checks below have something to work with.
    ...
    // Every item the caller sent must come back exactly once, either packed into a box or
    // listed as unplaced with a reason attached.
    for (const auto& item : items) {
